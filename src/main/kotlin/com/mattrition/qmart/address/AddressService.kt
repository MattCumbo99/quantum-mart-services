...
class AddressService(
     ...
     */
    fun getUserAddresses(userId: UUID): List<AddressDto> =
        addressRepository.findByUserIdSorted(userId)
    ...
    fun getUserPrimaryAddress(userId: UUID): AddressDto {
        val address = addressRepository.findPrimaryAddress(userId)
    ...
    fun deleteAddress(addressId: UUID) {
        val address = addressRepository.findById(addressId).getOrElse {
    ...
    private fun adjustPrimaryAddress(userId: UUID) {
        val addresses = addressRepository.findByUserIdSorted(userId).ifEmpty { return }
